...
String newLine = System.lineSeparator(); //Line separator for GUI outputs
	...
	//**********************//
	// Graphing GUI Objects //
	//**********************//
	JFrame graphWindow = new JFrame("Graphing Calculator"); //Window for Graphing Calculator
	JLabel gphLabel = new JLabel("Graphing Calculator");
	JLabel gphInputLabel = new JLabel("Expression: "); //Input Label for graphing
	JLabel gphIncrementLabel = new JLabel("Increment"); //Increment Label for graphing
	JButton gphclear = new JButton("Clear"); //Reset button for graphing
	JTextField gphInput = new JTextField(); //Input text area for graphing
	JTextField gphIncrement = new JTextField(); //Increment text area for graphing
	JTextField gphXArea = new JTextField(); // Area to enter x value
	JLabel gphXLabel = new JLabel("Starting X ="); //Label for x =
	JPanel gphCenter = new JPanel(); //Panel in middle of graphing GUI
	JPanel gphLeftCenter = new JPanel(); //Panel for labels
	JPanel gphRightCenter = new JPanel(); //Panel for input fields
	JPanel gphBottom = new JPanel(); //Panel at bottom of graphing GUI
	JPanel gphClearPanel = new JPanel(); //Panel for clear button
	JButton graphButton = new JButton("Graph"); //Button to graph inputs
	JPanel gphGraphPanel = new JPanel(); //Panel for graph button
	JPanel gphInputPanel = new JPanel();
	JPanel gphXPanel = new JPanel();
	JPanel gphIncrementPanel = new JPanel();
	JPanel Graph = new JPanel();
	JFrame GraphFrame = new JFrame("Hit 'Graph' button to Graph");
	Graphics graphics;
	...
	public GraphingCalculator(){
		...
		gphInputPanel.setBackground(new Color(0x4FD5D6));
		gphXPanel.setBackground(new Color(0xCDFFFF));
		gphIncrementPanel.setBackground(new Color(0x0099CC));
	...
	private void graphingGUI(){
		...
		graphWindow.setSize(600,350); //Set the window size
		graphWindow.setLocation(0,70); // Set the location of the window
		graphWindow.setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE); //Nothing happens if user tries to close
		//**************//
		// Bottom Panel //
		//**************//
		gphClearPanel.add(gphclear); //Adds Reset Button to Reset Panel
		errorPanel.add(errorLabelField); //Adds Error Label to Error Panel
		gphGraphPanel.add(graphButton); //Adds graph button to graph panel
		gphBottom.setLayout(new GridLayout(1,3)); //Set Layout for bottom panel of accumulator
		gphBottom.add(gphClearPanel); //Adds clear button panel to bottom panel
		gphBottom.add(gphGraphPanel); //Adds graph button panel to bottom panel
		gphBottom.add(errorPanel); //Adds error message panel to bottom panel
		graphWindow.getContentPane().add(gphBottom, "South"); //Adds bottom panel to Accumulator GUI
		errorLabelField.setForeground(Color.red); //Set error message color.
		//**************//
		// Center Panel //
		//**************//
		//Left Side
		gphLeftCenter.setLayout(new GridLayout(3,1));
		gphInputPanel.add(gphInputLabel);
		gphLeftCenter.add(gphInputPanel);
		gphInputLabel.setFont(new Font("default", Font.BOLD, 20));
		gphXPanel.add(gphXLabel);
		gphLeftCenter.add(gphXPanel);
		gphXLabel.setFont(new Font("default", Font.BOLD, 20));
		gphIncrementPanel.add(gphIncrementLabel);
		gphLeftCenter.add(gphIncrementPanel);
		gphIncrementLabel.setFont(new Font("default", Font.BOLD, 20));
		//Right Side
		gphRightCenter.setLayout(new GridLayout(3,1));
		gphRightCenter.add(gphInput);
		gphRightCenter.add(gphXArea);
		gphRightCenter.add(gphIncrement);
		//Add both sides to center
		gphCenter.setLayout(new GridLayout(1,2));
		gphCenter.add(gphLeftCenter);
		gphCenter.add(gphRightCenter);
		graphWindow.getContentPane().add(gphCenter, "Center"); //Add center panel to Accumulator GUI
		//Reveal everything and enable buttons
		graphButton.addActionListener(this);
		gphclear.addActionListener(this);
		graphWindow.setVisible(true);
		
		GraphFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); 
	      GraphFrame.setSize(600, 420);
	      GraphFrame.setLocation(600, 0);
	...
	public String calculate(String Expression, String x)
		...
		while(Expression.contains("r") || Expression.contains("^") || Expression.contains("*") || Expression.contains("/")){
			...
			if(neg)
				base = Double.parseDouble(Expression.substring(basestart, operator));
			else 
			...
			String exp2 = null;
			if(neg || neg2){
				if(basestart==-1) basestart = basestart+1;
				exp1 = Expression.substring(0,Expression.indexOf(Expression.substring(basestart,powerstart)));
				exp2 = Expression.substring(Expression.substring(basestart,powerstart).length() + exp1.length());
			}
			else{
				exp1 = Expression.substring(0,Expression.indexOf(Expression.substring(basestart+1,powerstart)));
				exp2 = Expression.substring(Expression.substring(basestart+1,powerstart).length() + exp1.length());
			}
	...
	public static void main(String[] args) {
		...
		new GraphingCalculator(); //Creation of the GUI
	}
	...
	public void actionPerformed(ActionEvent ae){
		if(ae.getSource() == gphclear){
			gphInput.setText("");
			gphXArea.setText("");
			gphIncrement.setText("");
		}
		if(ae.getSource() == graphButton){
			// 1. Graph 3 numbers/expressions
			String graphInput = gphInput.getText();
			String xArea = gphXArea.getText();
			String increment = gphIncrement.getText();
			
			// 2. check if 3 numbers/expressions are empty
			if(graphInput.trim().length()==0) {
				errorLabelField.setText("No expression given!");
				return;
			}
			if(xArea.trim().length()==0){
				errorLabelField.setText("No starting value given!");
				return;
			}
			if(increment.trim().length()==0) {
				errorLabelField.setText("No increment given!");	
				return;
			}
			
			// 3. check if x increment is positive
			try{
				drawGraph(graphInput, xArea, increment);
			}catch(Exception e){
	...
	public void drawGraph(String expression, String xStart, String increment)
			throws IllegalArgumentException {
		// use expression, the start, and the increment to create a set of x and y values calling the calculate function
		double[] x = new double[11];
		double[] y = new double[11];
		double xValue;
		double start = Double.parseDouble(xStart);
		double inc = Double.parseDouble(increment);
		...
		for(int i=0; i<=10;i++){
			String yValues = calculate(expression, Double.toString(x[i]));
			y[i] = Double.parseDouble(yValues);
		}
		
		setxIncrement(inc);
		
		// then call the graph function
		graphPanel = new GraphPanel(expression,x,y,this);
		graphics = Graph.getGraphics();
		graphPanel.paint(graphics);
		dim = graphPanel.getSize();
	}
